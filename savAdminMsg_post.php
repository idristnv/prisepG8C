...
while ($dernierticket = $ticketfinal->fetch()){
        if ($dernierticket['numTicket'] == $_POST['numTicketURL']){
                $req->execute(array('Administrateur Prisep', $dernierticket['numTicket'], $_POST['texte'])); //mail a remplacer par la session de l'individu
        }
}
...
header("Location: savAdminMsg.php?ticket=".$fticket);
?>